...
The pure-API donation tests (`tests/api/donations.api.spec.ts`) need a guest
**registered on the E2E event with a card pre-authorised** to call
`checkin.makeDonation` against. `E2E_API_GUEST_ID` (`.env`) seeds this — it
was originally created once by hand through the Lite UI registration flow on
2026-09-06 ("QA E2E Donor 1335") — but `api-setup` now heals it automatically
(`src/api/guestFixture.ts`'s `ensureApiGuest`, wired into
`tests/setup/api.setup.ts`): it checks whether the current guest id still
resolves (`checkout()` 404s for an unknown guest; `paymentTransactions()`
doesn't, so it can't be used for this check) and, only if it's gone, launches
a real Chromium browser to run the same Donate → sign-in → register → opt-ins
flow the e2e specs use. **A browser only, because it has to be** — guest
creation (`POST lite/v1/events/:id/auth/guests`) requires a `captchaToken`
field, a real reCAPTCHA Enterprise token generated by executing Google's JS
challenge on the live page (verified live 2026-09-16 by capturing the actual
request body); there is no pure-API way to obtain one. Unlike the ticket/lot/
raffle fixtures, a deleted guest can't be healed back under its old id — a
fresh registration always gets a new server-assigned UUID — so the resolved
id is persisted to `playwright/.auth/e2e-api-guest.json` (gitignored, mirrors
how the EMS token itself is threaded through) and the `e2eEvent` fixture
reads it in preference to the static `.env` value. The common case (guest
