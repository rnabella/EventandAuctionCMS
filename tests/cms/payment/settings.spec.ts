...
test.describe.serial('CMS Payment Collection > Settings', () => {
  test('enables "Allow donor tip or platform fee" and it persists (checklist: Review the payment method - Donor Tip or Platform fee)', async ({
    page,
  }) => {
  ...
  test('renders the DAFpay section (checklist: Set up the ability for DAF Pay — structural check only, see class docs)', async ({
    page,
  }) => {
    const paymentSettingsPage = new PaymentSettingsPage(page);
    await paymentSettingsPage.goto(env.testEventId);
